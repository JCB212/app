...
public class FiscalApp extends Application {
    ...
    private VBox buildSidebar() {
        ...
        Button btnHist    = menuBtn("Histórico/Cache",      ICON_HIST,    "historico");
        Button btnConfig  = menuBtn("Configurações",       ICON_SETTINGS,"config");

        menu.getChildren().addAll(btnHome, btnProcess, btnReproc, btnDest, btnEmail, btnHist, btnConfig);
    ...
    private void showHistorico() {
        try {
            ensureDbConnected();
            HistoricoApp historico = new HistoricoApp(dbGateway.getConnection());
            contentArea.getChildren().setAll(historico);
            historico.pesquisar();
        } catch (Exception e) {
            log.error("Erro ao abrir histórico", e);
            showAlert("Erro ao carregar histórico: " + e.getMessage() +
                "\n\nVerifique a conexão com o banco de dados.", Alert.AlertType.ERROR);
        }
    }
